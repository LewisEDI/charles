...
def test_loan_money
  loan_money(@person5, @person4, 20)
  lender_result = @person5[:monies]
  lender_expected = 80
  borrower_result = @person4[:monies]
  borrower_expected = 40
  assert_equal(lender_expected, lender_result)
  assert_equal(borrower_expected, borrower_result)
end
...
def test_all_snacks
  snack_list = ["charcuterie", "soup", "bread", "Scooby snacks", "spaghetti", "ratatouille", "spinach"]
  ...
  def test_no_friends
    no_friends_list = ["Daphne"]
    result = no_friends(@people)
    expected = no_friends_list
    assert_equal(expected, result)
  end
